...
export class GameHudScreen {
  ...
  public constructor(data: { day: number; money: number; time: GameTime; district: string; name: string }) {
    ...
    this.time = el('span', { class: 'hud__time', text: this.formatTime(data.time) });
    this.day = el('span', { class: 'hud__day-value', text: t('game.day', { day: data.day }) });
    this.district = el('span', { class: 'hud__district', text: data.district });
    ...
    this.root = el('div', { class: 'screen screen--game' }, [
      ...
      el('div', { class: 'hud__info' }, [this.day, this.district]),
  ...
  public updateTime(time: GameTime): void { this.time.textContent = this.formatTime(time); this.day.textContent = t('game.day', { day: time.day }); }
  private formatTime(time: GameTime): string { return `${String(time.hour).padStart(2, '0')}:${String(time.minute).padStart(2, '0')}`; }

  private createJoystick(): HTMLElement {
    const base = el('div', { class: 'joystick' }); const knob = el('div', { class: 'joystick__knob' }); base.append(knob); const radius = 46;
    const update = (event: PointerEvent): void => { const rect = base.getBoundingClientRect(); const dx = event.clientX - (rect.left + rect.width / 2); const dy = event.clientY - (rect.top + rect.height / 2); const length = Math.hypot(dx, dy); const scale = length > radius ? radius / length : 1; const x = (dx * scale) / radius; const y = (dy * scale) / radius; if (Math.hypot(x, y) < 0.16) { bus.emit('ui:joystick', { x: 0, y: 0 }); knob.style.transform = 'translate(-50%, -50%)'; return; } knob.style.transform = `translate(calc(-50% + ${x * radius}px), calc(-50% + ${y * radius}px))`; bus.emit('ui:joystick', { x, y }); };
